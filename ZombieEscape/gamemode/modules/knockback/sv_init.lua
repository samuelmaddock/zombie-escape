...
CVars.PropKnockback 		= CreateConVar( "ze_propForceMultiplier", '2.0', {FCVAR_REPLICATED}, "Force multiplier for props when shot." )
...
*/
local function PropPhysicsKnockback( ent, dmginfo )

	local inflictor = dmginfo:GetInflictor()
	local attacker = dmginfo:GetAttacker()
	local amount = dmginfo:GetDamage()

	--Check that the entity is of the type we are looking for
	if !IsValid(ent) or !IsValid(inflictor) or ent:IsPlayer() or ent:IsNPC() then return end
	...
	local forceMultiplier = CVars.PropKnockback:GetFloat() * weapMult
...
hook.Add( "EntityTakeDamage", "PropKnockback", PropPhysicsKnockback )